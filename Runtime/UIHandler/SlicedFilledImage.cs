...
#pragma warning disable 0649
        ...
        public FillDirection fillDirection
        {
            ...
            set
            {
                if (SetPropertyUtility.SetStruct(ref m_FillDirection, value)) SetVerticesDirty();
            }
        }
...
#pragma warning restore 1692
        ...
        private void GenerateSlicedFilledSprite(VertexHelper vh)
        {
            ...
            if (!hasBorder)
            {
                ...
                Vector4 vertices = new Vector4(rect.x + rect.width * (padding.x / spriteW),
            ...
            for (int x = 0; x < 3; x++)
            {
                ...
                for (int y = 0; y < 3; y++)
                {
                    ...
                    if (sliceStart >= m_FillAmount) continue;
        ...
        private void GenerateFilledSprite(VertexHelper vh, Vector4 vertices, Vector4 uvs, float fillAmount)
        {
            ...
            for (int i = 0; i < 4; i++) vh.AddVert(s_Vertices[i], color, s_UVs[i]);
        ...
        bool ICanvasRaycastFilter.IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
        {
            ...
            {
                case FillDirection.Right:
                    if (x > m_FillAmount) return false;
                    break;
                case FillDirection.Left:
                    if (1f - x > m_FillAmount) return false;
                    break;
                case FillDirection.Up:
                    if (y > m_FillAmount) return false;
                    break;
                case FillDirection.Down:
                    if (1f - y > m_FillAmount) return false;
                    break;
            }
